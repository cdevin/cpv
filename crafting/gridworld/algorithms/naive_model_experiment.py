...
import numpy as np
import copy
from gridworld.algorithms.composite_models import NaiveModel
...
parser.add_argument('--tasks', type=str, nargs="*", default='EatBreadPolicy',
...
result_list = []
model = NaiveModel(device,task_embedding_dim=args.feature_dim).to(device)
...
def train(epoch):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        logpi =model.forward(first_image, image, pre_image, post_image)
    ...
    writer.add_scalar('Regloss/train',feat_loss_total/num_batch, epoch)
...
if __name__ == "__main__":
    filename = 'naive_4tasks'+'_images_feat'+str(args.feature_dim)+'_'+args.letter+'_{:06d}'.format(args.train_size)
